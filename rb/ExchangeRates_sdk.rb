...
class ExchangeRatesSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = ExchangeRatesHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = ExchangeRatesHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, ExchangeRatesFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    ExchangeRatesUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    params = ExchangeRatesHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = ExchangeRatesHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = ExchangeRatesHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = ExchangeRatesError.new(
        "graphql_error", "ExchangeRatesSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = ExchangeRatesSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
