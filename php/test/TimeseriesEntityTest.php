...
class TimeseriesEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = ExchangeRatesSDK::test(null, null);
        $ent = $testsdk->Timeseries(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = timeseries_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "timeseries." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set EXCHANGE_RATES_TEST_TIMESERIES_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $timeseries_ref01_data = null;
        if (count($timeseries_ref01_data_raw) > 0) {
...
function timeseries_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/timeseries/TimeseriesTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["timeseries01", "timeseries02", "timeseries03"] as $k) {
    ...
    $entid_env_raw = getenv("EXCHANGE_RATES_TEST_TIMESERIES_ENTID");
