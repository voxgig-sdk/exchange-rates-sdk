...
class SymbolEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = ExchangeRatesSDK::test(null, null);
        $ent = $testsdk->Symbol(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = symbol_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "symbol." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set EXCHANGE_RATES_TEST_SYMBOL_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $symbol_ref01_data = null;
        if (count($symbol_ref01_data_raw) > 0) {
...
function symbol_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/symbol/SymbolTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["symbol01", "symbol02", "symbol03"] as $k) {
    ...
    $entid_env_raw = getenv("EXCHANGE_RATES_TEST_SYMBOL_ENTID");
