...
class StatusEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set EXCHANGERATES_TEST_STATUS_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function status_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("EXCHANGERATES_TEST_STATUS_ENTID");
    $idmap_overridden = $entid_env_raw !== false && str_starts_with(trim($entid_env_raw), "{");

    $env = Runner::env_override([
        "EXCHANGERATES_TEST_STATUS_ENTID" => $idmap,
        "EXCHANGERATES_TEST_LIVE" => "FALSE",
        "EXCHANGERATES_TEST_EXPLAIN" => "FALSE",
    ]);

    $idmap_resolved = Helpers::to_map(
        $env["EXCHANGERATES_TEST_STATUS_ENTID"]);
