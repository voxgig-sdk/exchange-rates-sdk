...
func (sdk *ExchangeRatesSDK) opAllowed(op string) bool {
	allowOp, _ := vs.GetPath(sdk.options, []any{"allow", "op"}).(string)
	return strings.Contains(allowOp, op)
}

func (sdk *ExchangeRatesSDK) opDenied(op string) map[string]any {
	allowOp, _ := vs.GetPath(sdk.options, []any{"allow", "op"}).(string)
	return map[string]any{
		"ok": false,
		"err": fmt.Errorf("ExchangeRatesSDK: %s: operation not allowed by"+
...
// gate by passing it.
func (sdk *ExchangeRatesSDK) rawRequest(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *ExchangeRatesSDK) Graphql(
	...
	if 0 < len(errors) {
		...
		res["ok"] = false
		res["err"] = fmt.Errorf("ExchangeRatesSDK: graphql: %s", msg)
		res["graphql"] = errors
	}
