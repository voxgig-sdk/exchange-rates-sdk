...
import (
	...
	omni "github.com/voxgig-sdk/exchange-rates-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.ExchangeRatesError); is {
				out := map[string]any{
					"name":    "ExchangeRatesError",
...
func omniCtx(arg any, client *sdk.ExchangeRatesSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.ExchangeRatesSDK); is {
			client = live
