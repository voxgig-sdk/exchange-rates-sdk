...
func TestGetApiRootEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set EXCHANGE_RATES_TEST_GET_API_ROOT_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func get_api_rootBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("EXCHANGE_RATES_TEST_GET_API_ROOT_ENTID")
	...
	idmapResolved := core.ToMapAny(env["EXCHANGE_RATES_TEST_GET_API_ROOT_ENTID"])
