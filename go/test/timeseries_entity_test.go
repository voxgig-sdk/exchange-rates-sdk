...
func TestTimeseriesEntity(t *testing.T) {
	t.Run("instance", func(t *testing.T) {
		...
		if ent == nil {
			t.Fatal("expected non-nil TimeseriesEntity")
		}
	})

	t.Run("basic", func(t *testing.T) {
		setup := timeseriesBasicSetup(nil)
		...
		for _, _op := range []string{"load"} {
			if _shouldSkip, _reason := isControlSkipped("entityOp", "timeseries." + _op, _mode); _shouldSkip {
				if _reason == "" {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set EXCHANGE_RATES_TEST_TIMESERIES_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		_ = timeseriesRef01Data

		// LOAD
		timeseriesRef01Ent := client.Timeseries(nil)
		timeseriesRef01MatchDt0 := map[string]any{}
		timeseriesRef01DataDt0Loaded, err := timeseriesRef01Ent.Load(timeseriesRef01MatchDt0, nil)
		if err != nil {
			t.Fatalf("load failed: %v", err)
		}
		if timeseriesRef01DataDt0Loaded == nil {
...
func timeseriesBasicSetup(extra map[string]any) *entityTestSetup {
	loadEnvLocal()

	_, filename, _, _ := runtime.Caller(0)
	dir := filepath.Dir(filename)

	entityDataFile := filepath.Join(dir, "..", "..", ".sdk", "test", "entity", "timeseries", "TimeseriesTestData.json")

	entityDataSource, err := os.ReadFile(entityDataFile)
	if err != nil {
		panic("failed to read timeseries test data: " + err.Error())
	}

	var entityData map[string]any
	if err := json.Unmarshal(entityDataSource, &entityData); err != nil {
		panic("failed to parse timeseries test data: " + err.Error())
	}
	...
	entidEnvRaw := os.Getenv("EXCHANGE_RATES_TEST_TIMESERIES_ENTID")
