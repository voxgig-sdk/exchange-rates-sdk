-- ExchangeRates SDK GetHistoricalRateForCurrencyAndDate entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local GetHistoricalRateForCurrencyAndDateEntity = {}
GetHistoricalRateForCurrencyAndDateEntity.__index = GetHistoricalRateForCurrencyAndDateEntity


function GetHistoricalRateForCurrencyAndDateEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, GetHistoricalRateForCurrencyAndDateEntity)
  self._name = "get_historical_rate_for_currency_and_date"
  self._client = client
...
function GetHistoricalRateForCurrencyAndDateEntity:get_name()
  return self._name
end


function GetHistoricalRateForCurrencyAndDateEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return GetHistoricalRateForCurrencyAndDateEntity.new(self._client, opts)
end
...
function GetHistoricalRateForCurrencyAndDateEntity:mark_deleted()
  self._deleted = true
end


function GetHistoricalRateForCurrencyAndDateEntity:deleted()
  return true == self._deleted
end


function GetHistoricalRateForCurrencyAndDateEntity:data_set(args)
  if args ~= nil then
...
function GetHistoricalRateForCurrencyAndDateEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function GetHistoricalRateForCurrencyAndDateEntity:match_set(args)
  if args ~= nil then
...
function GetHistoricalRateForCurrencyAndDateEntity:match_get()
...
function GetHistoricalRateForCurrencyAndDateEntity:stream(action, args, callopts)
...
---@param reqmatch GetHistoricalRateForCurrencyAndDateLoadMatch
---@param ctrl? table
---@return GetHistoricalRateForCurrencyAndDate
---@return string? err
function GetHistoricalRateForCurrencyAndDateEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function GetHistoricalRateForCurrencyAndDateEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return GetHistoricalRateForCurrencyAndDateEntity
