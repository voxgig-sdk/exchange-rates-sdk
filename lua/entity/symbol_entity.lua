...
local SymbolEntity = {}
SymbolEntity.__index = SymbolEntity


function SymbolEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, SymbolEntity)
  self._name = "symbol"
  self._client = client
...
function SymbolEntity:get_name()
  return self._name
end


function SymbolEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return SymbolEntity.new(self._client, opts)
end
...
function SymbolEntity:mark_deleted()
  self._deleted = true
end


function SymbolEntity:deleted()
  return true == self._deleted
end


function SymbolEntity:data_set(args)
  if args ~= nil then
...
function SymbolEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function SymbolEntity:match_set(args)
  if args ~= nil then
...
function SymbolEntity:match_get()
...
function SymbolEntity:stream(action, args, callopts)
...
---@param reqmatch SymbolLoadMatch
---@param ctrl? table
---@return Symbol
---@return string? err
function SymbolEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function SymbolEntity:_run_op(ctx, post_done)
  local utility = self._utility
