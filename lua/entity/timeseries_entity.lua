...
local TimeseriesEntity = {}
TimeseriesEntity.__index = TimeseriesEntity


function TimeseriesEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, TimeseriesEntity)
...
function TimeseriesEntity:get_name()
  return self._name
end


function TimeseriesEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return TimeseriesEntity.new(self._client, opts)
end
...
function TimeseriesEntity:mark_deleted()
  self._deleted = true
end


function TimeseriesEntity:deleted()
  return true == self._deleted
end


function TimeseriesEntity:data_set(args)
  if args ~= nil then
...
function TimeseriesEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function TimeseriesEntity:match_set(args)
  if args ~= nil then
...
function TimeseriesEntity:match_get()
...
function TimeseriesEntity:stream(action, args, callopts)
...
function TimeseriesEntity:_run_op(ctx, post_done)
  local utility = self._utility
