...
-- by passing it.
function ExchangeRatesSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function ExchangeRatesSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "ExchangeRatesSDK: graphql: " .. msg
    res.graphql = errors
  end
