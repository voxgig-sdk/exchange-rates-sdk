...
class TestTimeseriesDirect:

    def test_should_direct_load_timeseries(self):
        setup = _timeseries_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-timeseries", "live" if setup["live"] else "unit")
        if _skip:
...
def _timeseries_direct_setup(mockres):
