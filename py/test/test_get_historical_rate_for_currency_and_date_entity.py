    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set EXCHANGE_RATES_TEST_GET_HISTORICAL_RATE_FOR_CURRENCY_AND_DATE_ENTID JSON to run live")
        client = setup["client"]
...
def _get_historical_rate_for_currency_and_date_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("EXCHANGE_RATES_TEST_GET_HISTORICAL_RATE_FOR_CURRENCY_AND_DATE_ENTID"))
