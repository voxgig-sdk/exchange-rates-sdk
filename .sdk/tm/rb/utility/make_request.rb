# ExchangeRates SDK utility: make_request
...
module ExchangeRatesUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = ExchangeRatesResponse.new({})
    ...
    if fetch_err
      ...
      response = ExchangeRatesResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = ExchangeRatesResponse.new(fetched)
    else
