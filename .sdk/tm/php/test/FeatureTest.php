...
// ExchangeRates SDK feature test
//
...
class FtCtrl extends ExchangeRatesControl
{
...
class FtClient
{
    ...
    private ExchangeRatesUtility $utility;

    public function __construct(ExchangeRatesUtility $utility, array $options)
    {
    ...
    public function get_utility(): ExchangeRatesUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (ExchangeRatesContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public ExchangeRatesContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (ExchangeRatesContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = ExchangeRatesConfig::make_feature($name);
    ...
    private static function buildUrl(ExchangeRatesSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof ExchangeRatesError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new ExchangeRatesResponse($fetched);
        }
    ...
    private function fail(ExchangeRatesContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (ExchangeRatesContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
