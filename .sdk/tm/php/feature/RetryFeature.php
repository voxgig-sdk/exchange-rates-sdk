...
// ExchangeRates SDK retry feature
...
class ExchangeRatesRetryFeature extends ExchangeRatesBaseFeature
{
    ...
    public function init(ExchangeRatesContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (ExchangeRatesContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(ExchangeRatesContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(ExchangeRatesContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
