...
class ExchangeRatesMakeRequest
{
    public static function call(ExchangeRatesContext $ctx): array
    {
        ...
        if ($fetch_err) {
            ...
            $response = new ExchangeRatesResponse(['err' => $ctx->make_error('request_no_response', 'response: undefined')]);
        } elseif (is_array($fetched)) {
            $response = new ExchangeRatesResponse($fetched);
        } else {
