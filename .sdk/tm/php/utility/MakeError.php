...
class ExchangeRatesMakeError
{
    public static function call(?ExchangeRatesContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new ExchangeRatesContext([], null);
        }
        $op = $ctx->op ?? new ExchangeRatesOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new ExchangeRatesResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof ExchangeRatesError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new ExchangeRatesError('', $msg, $ctx);
