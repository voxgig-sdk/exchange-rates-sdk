...
boru-driven command-line client **and** interactive REPL for the ExchangeRates
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/exchange-rates-cli)
make build

# 2. See usage (words, entities, env vars)
./exchange-rates-cli --help

# 3. Provide credentials once, via the environment
export EXCHANGE_RATES_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./exchange-rates-cli load 1 convert            # {id:1} shorthand
./exchange-rates-cli load '{id:1}' convert       # explicit match map

# 5. Override the API base URL for a single call
EXCHANGE_RATES_BASE=https://api.example.com ./exchange-rates-cli load 1 convert

# 6. No arguments -> interactive REPL
./exchange-rates-cli
exchange-rates> load 1 convert
exchange-rates> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/exchange-rates-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/exchange-rates-cli load 1 convert
   ```
...
```sh
./exchange-rates-cli load 1 convert          # scalar shorthand for {id:1}
./exchange-rates-cli load '{id:1}' convert     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `exchange-rates>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/exchange-rates-cli`. |
...
(`./exchange-rates-cli <expr>`) and interactively (the REPL), and expressions compose the
