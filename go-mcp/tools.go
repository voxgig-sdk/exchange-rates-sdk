...
type Args struct {
	Entity string         `json:"entity" jsonschema:"convert | get_api_root | get_historical_rate_for_currency_and_date | get_historical_rates_for_date | latest | status | symbol | timeseries"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.ExchangeRatesSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "exchange-rates_list",
		Description: "List records from ExchangeRates. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "exchange-rates_load",
		Description: "Load a single record from ExchangeRates. " +
...
func runOp(client *sdk.ExchangeRatesSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.ExchangeRatesSDK, name string) (sdk.ExchangeRatesEntity, error) {
	switch strings.ToLower(name) {
	case "convert":
		return client.Convert(nil), nil
	case "get_api_root":
		return client.GetApiRoot(nil), nil
	case "get_historical_rate_for_currency_and_date":
		return client.GetHistoricalRateForCurrencyAndDate(nil), nil
	case "get_historical_rates_for_date":
		return client.GetHistoricalRatesForDate(nil), nil
	case "latest":
		return client.Latest(nil), nil
	case "status":
		return client.Status(nil), nil
	case "symbol":
		return client.Symbol(nil), nil
	case "timeseries":
		return client.Timeseries(nil), nil
