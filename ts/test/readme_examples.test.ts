...
//     (\`new ExchangeRatesSDK(...)\` / \`ExchangeRatesSDK.test(...)\` become
//     \`ExchangeRatesSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'ExchangeRatesSDK'
...
const TEST_SEED = {"entity":{"convert":{"test01":{"id":"test01"}},"get_api_root":{"test01":{"id":"test01"}},"get_historical_rate_for_currency_and_date":{"test01":{"id":"test01"}},"get_historical_rates_for_date":{"test01":{"id":"test01"}},"latest":{"test01":{"id":"test01"}},"status":{"test01":{"id":"test01"}},"symbol":{"test01":{"id":"test01"}},"timeseries":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new ExchangeRatesSDK(...)\` and any
// \`ExchangeRatesSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+ExchangeRatesSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/ExchangeRatesSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
