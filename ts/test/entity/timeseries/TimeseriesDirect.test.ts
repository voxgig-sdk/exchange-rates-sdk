...
describe('TimeseriesDirect', async () => {
  ...
  test('direct-load-timeseries', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-timeseries', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['EXCHANGE_RATES_TEST_TIMESERIES_ENTID']
