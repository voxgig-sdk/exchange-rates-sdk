...
describe('GetHistoricalRatesForDateDirect', async () => {
  ...
  test('direct-load-get_historical_rates_for_date', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-get_historical_rates_for_date', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['EXCHANGE_RATES_TEST_GET_HISTORICAL_RATES_FOR_DATE_ENTID']
