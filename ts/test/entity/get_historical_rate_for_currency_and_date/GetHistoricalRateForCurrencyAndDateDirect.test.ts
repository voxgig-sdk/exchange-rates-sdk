...
describe('GetHistoricalRateForCurrencyAndDateDirect', async () => {
  ...
  test('direct-exists', async () => {
    const sdk = new ExchangeRatesSDK({
  ...
  test('direct-load-get_historical_rate_for_currency_and_date', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-get_historical_rate_for_currency_and_date', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['EXCHANGE_RATES_TEST_GET_HISTORICAL_RATE_FOR_CURRENCY_AND_DATE_ENTID']
