...
(0, node_test_1.describe)('GetHistoricalRateForCurrencyAndDateDirect', async () => {
    ...
    (0, node_test_1.test)('direct-exists', async () => {
        const sdk = new __1.ExchangeRatesSDK({
    ...
    (0, node_test_1.test)('direct-load-get_historical_rate_for_currency_and_date', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-get_historical_rate_for_currency_and_date', setup.live))
            return;
...
function liveScenariosActive() { return false && process.env.EXCHANGE_RATES_TEST_LIVE === 'TRUE'; }
function directSetup(mockres) {
    const calls = [];
    const env = (0, utility_1.envOverride)({
        'EXCHANGE_RATES_TEST_GET_HISTORICAL_RATE_FOR_CURRENCY_AND_DATE_ENTID': {},
    ...
    if (live) {
        ...
        const client = new __1.ExchangeRatesSDK(Object.assign({}, (0, utility_1.liveClientOptions)(), { system: { fetch: transport.fetch },
            apikey: env.EXCHANGE_RATES_APIKEY,
        }));
        let idmap = env['EXCHANGE_RATES_TEST_GET_HISTORICAL_RATE_FOR_CURRENCY_AND_DATE_ENTID'];
    ...
    const client = new __1.ExchangeRatesSDK({
        base: 'http://localhost:8080',
...
//# sourceMappingURL=GetHistoricalRateForCurrencyAndDateDirect.test.js.map